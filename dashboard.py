            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                if metmessage.selected_format == 1:
                    ...
                    for files in file_list:
                        ...
                        output_directory = selected_file.replace('/Text files (*.txt)', '')
                if metmessage.selected_format == 4:
                    create_METCM.acceptance_flag = 0
                    selected_directory = dialog.selectedFiles()[0]
                    file_name = dialog.selectedNameFilter()  # Get the specified file name
                    if selected_directory:
    ...
    def save_file_dialog(self):
        ...
        try:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                create_METTA.acceptance_flag = 0
                create_METCM.acceptance_flag = 0
                create_METAV.acceptance_flag = 0
                selected_directory = dialog.selectedFiles()[0]
                if selected_directory:
                    output_files = []
                    for files in file_list:
                        ...
                        selected_file_metav = f"{selected_directory}/{formatted_date}-{formatted_time}UTC-METAV.txt"

                        output_files.append(selected_file_metta)
                        output_files.append(selected_file_metcm)
                        output_files.append(selected_file_metav)

                    print(output_files)
            else:
                create_METCM.acceptance_flag = 1
                create_METTA.acceptance_flag = 1
                create_METAV.acceptance_flag = 1
...
class Ui_Dialog(object):
    ...
    def generate_MET(self):
        ...
        for header in input_header:
            ...
            create_METAV.forecast_dates.append(header['ReleaseDate'])
            create_METTA.forecast_times.append(header['ReleaseTime'])
            create_METCM.forecast_times.append(header['ReleaseTime'])
            create_METAV.forecast_times.append(header['ReleaseTime'])
        ...
        create_METAV.index = self.combobox.currentIndex()
        ...
        if metmessage.selected_format == 1:
            ...
            for i in range(len(file_list)):
                create_METTA.create_message(input_header[i], input_table[i], output_files[3*i])
                create_METCM.create_message(input_header[i], input_table[i], output_files[3*i+1])
                create_METAV.create_message(input_header[i], input_table[i], output_files[3*i+2])
        elif metmessage.selected_format == 4:
            print('METAV Selected')
            FileManagerDialog()
            create_METAV.create_message(input_header[self.combobox.currentIndex()], input_table[self.combobox.currentIndex()], output_directory)
        else:
